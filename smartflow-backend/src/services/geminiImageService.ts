...
export const getTraffic = async (
  base64ImageFileA: string,
  base64ImageFileB: string,
  base64ImageFileC: string,
  base64ImageFileD: string,
): Promise<trafficStateOfSquare | undefined> => {
  try {
    if (
      !isValidBase64(base64ImageFileA) ||
...
- do not change the JSON structure` },
    ...
    const parsedJSON: trafficStateOfSquare = JSON.parse(text)
